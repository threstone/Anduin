...
function doTest(args: any[], desc: string) {
    // it(`${desc} rpc request encode&decode 字段验证`, () => {
    {
    ...
    // it(`${desc} rpc result encode&decode json buffer字段验证`, () => {
    {
    ...
    // it(`${desc} rpc request encode&decode json buffer耗时比对`, () => {
    {
        const times = 10000;
        let lenAvg = 0;
        let now = Date.now();
        ...
        console.log(`\n${desc}   JSON  ${times}次序列化反序列化用时 : ${Date.now() - now}ms 序列化用时 : ${encodeTime} 反序列化用时:${decodeTime} 平均包体大小 : ${lenAvg / times}`);
        ...
        for (let index = 0; index < times; index++) {
            const encodeStart = Date.now();
            const buffer = RpcUtilsByBuffer.encodeCallReqest('nodeId', 'serverName', 'className', 'funcName', 2, { type: 1, nodeId: 'asd' }, args);
            encodeTime += (Date.now() - encodeStart);
            const decodeStart = Date.now();
        ...
        console.log(`${desc}  BUFFER ${times}次序列化反序列化用时 : ${Date.now() - now}ms 序列化用时 : ${encodeTime} 反序列化用时:${decodeTime} 平均包体大小 : ${lenAvg / times}`);
        console.log();
    }
    // });

    // it(`${desc} protobuf json对比`, () => {
    {
        let req = {
            type: 2,
            requestId: 2,
        ...
        console.log(`${desc}  protobuf ${times}次序列化反序列化用时 : ${Date.now() - now}ms 序列化用时 : ${encodeTime} 反序列化用时:${decodeTime} 平均包体大小 : ${lenAvg / times}`);
        console.log();
    }
    // });

    // it(`${desc} rpc result encode&decode json buffer耗时比对`, () => {
    {
        const times = 10000;
        let lenAvg = 0;
        let now = Date.now();
        ...
        console.log(`\n${desc}  JSON   结果${times}次序列化反序列化用时 : ${Date.now() - now}ms   平均包体大小${lenAvg / times}`);
        ...
        console.log(`${desc}  BUFFER 结果${times}次序列化反序列化用时 : ${Date.now() - now}ms   平均包体大小${lenAvg / times}`);
        console.log();
    }
    // });
    ...
    // it(`${desc} 全流程时间测试`, () => {
    {
        ...
        console.log(`\n${desc}   JSON  ${times}次全流程时间测试用时 : ${Date.now() - now}ms 平均包体大小 : ${lenAvg / times}`);
        ...
        console.log(`${desc}  BUFFER ${times}次全流程时间测试用时 : ${Date.now() - now}ms 平均包体大小 : ${lenAvg / times}`);
        console.log();
    }
    // })
}
...
describe('rpc test', () => {
    ...
    for (let index = 0; index < 1000; index++) {
        args.push(['something string string 按时打算给', Math.random() * index, Math.floor(Math.random() * index), false][Math.floor(Math.random() * 4)])
    }
    doTest(args, '复杂参数');
});