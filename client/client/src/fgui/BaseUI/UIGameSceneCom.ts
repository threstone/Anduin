...
module BaseUI {
	...
	export class UIGameSceneCom extends fairygui.GComponent {
		...
		protected constructFromXML(xml:any):void {
			...
			this.selfInfoBox = <UIUserInfoBox>(this.getChildAt(2));
			this.targetInfoBox = <UIUserInfoBox>(this.getChildAt(3));
			this.selfHand = <UIHandCardsCom>(this.getChildAt(4));
			this.targetHand = <UIHandCardsCom>(this.getChildAt(5));
			this.rightCtrl = <UIRightCtrlCom>(this.getChildAt(6));
			this.close = <UIButton3>(this.getChildAt(7));
			this.map = <UIMapView>(this.getChildAt(8));
		}
	}
}